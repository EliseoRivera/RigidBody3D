...
///Copyright (C) <2017>  <Eliseo Rivera> curso.tareas@gmail.com
  void ClaseOpenGL::DibujarCirculo(float h, float k, float r){
      int n=50;
      float x,y;
      float dt=2*PI/n;
...
void ClaseOpenGL::renderizar(){
    ...
    Drawarrow3D({0,0,0},{1000,0,0},rojo,0.003,.01);
    Drawarrow3D({0,0,0},{0,1000,0},verde,0.003,.01);
    Drawarrow3D({0,0,0},{0,0,1000},azul,0.003,.01);
/// //////////////////////
//DibujarCirculo(13,13,6);
SSRMS.renderizar();
//s1.renderizar();
}
...
void ClaseOpenGL::Resize(int width, int height){
  ...
  glViewport(0, 0, width, height);		// reset the viewport to new dimensions
        ...
        gluPerspective(52.0f,(GLdouble)width/(GLdouble)height,1.0f,10000.0f);
